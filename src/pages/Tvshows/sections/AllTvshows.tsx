...
function AllTvshows() {
  ...
  const {tvshowsData,  genreList, selectedGenre, setSelectedGenre, page, setPage, selectedGenreTvshows} = useMovieApi()
      ...
      >
        ...
        <Box display="grid" gridTemplateColumns={{base: "repeat(2, 1fr)", md: "repeat(4, 1fr)", lg: "repeat(6, 1fr)"}} gap="1rem">
          {displayAll?
            ...
            tvshowsData && tvshowsData.map((movieItem) => (
              <MovieCardComponent key={movieItem.id} movieItem={movieItem} type="tv"/> 
            ))
            :
            selectedGenreTvshows && selectedGenreTvshows.map((movieItem) => (
              <MovieCardComponent key={movieItem.id} movieItem={movieItem} type="tv"/> 
            ))    
